...
// Enhanced signIn using Passport.js local strategy
export const signIn = asyncHandler<ISignInRequest, Response>(
  async (req, res, next) => {
    // Use Passport.js local authentication
    passport.authenticate('local', async (err: any, user: UserDocument, info: any) => {
      if (err) {
        return next(err);
      }
      ...
      // Optional: Login user in session (if using sessions)
      if (req.logIn) {
...
// Enhanced logout functionality using Passport.js
export const logout = asyncHandler<any, Response>(
  async (req, res) => {
    ...
    // Passport.js logout (if session is available)
    if (req.logout) {
      req.logout((err: any) => {
        if (err) {
    ...
    // Destroy session (if session is available)
